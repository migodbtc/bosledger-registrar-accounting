...
const gradingCriteria = [
  {
    key: "functionalAccuracy" as keyof GradingCriteria,
    label: "Functional Accuracy",
  },
  {
    key: "systemStability" as keyof GradingCriteria,
    label: "System Stability & Error Handling",
  },
  {
    key: "uiuxConsistency" as keyof GradingCriteria,
    label: "UI/UX Consistency",
  },
  {
    key: "securityAccess" as keyof GradingCriteria,
    label: "Security & Access Control",
  },
  {
    key: "performanceResponsiveness" as keyof GradingCriteria,
...
export const GradingForm = ({
  ...
  const handleSubmit = () => {
    ...
    // Optimistically submit to UI
    onSubmit(result);

    // Try to persist the result to the local save-result server (dev helper).
    // This is best-effort: failures won't block UI flow.
    (async () => {
      try {
        const resp = await fetch(
          process.env.VITE_SAVE_SERVER_URL ||
            "http://localhost:5002/save-result",
          {
            method: "POST",
        ...
        if (!resp.ok) {
          ...
          console.warn("Save server responded with non-ok", payload);
          toast.error("Result saved locally failed (server error)");
          return;
        }

        const data = await resp.json().catch(() => ({}));
        // data.file contains the saved filename
        toast.success("Result autosaved to workspace results folder");
        console.debug("Saved result file:", data.file || data);

        // Try to fetch the saved file content from the dev server so we can
        // update the in-memory history with the persisted result (adds savedFile)
        if (data && data.file) {
          try {
            // Fetch from the dev-server-served /results path
            const savedUrl = `${
              window.location.origin
            }/results/${encodeURIComponent(data.file)}`;
            const savedResp = await fetch(savedUrl);
            if (savedResp.ok) {
              const savedJson = await savedResp.json();
              // annotate with the saved filename so UI can show it
              const persisted = { ...savedJson, savedFile: data.file };
              // Notify parent with the persisted result (will dedupe/merge by id)
              onSubmit(persisted as any);
            } else {
              console.warn("Saved file not immediately available at", savedUrl);
            }
          } catch (err) {
            console.warn("Failed to fetch saved file content", err);
          }
        }
      } catch (err) {
        console.warn("Could not reach save-result server", err);
        toast.error(
          "Autosave failed — run the local save server to persist results"
        );
      }
    })();
  };
